...
func TestSecureHeaders(t *testing.T) {
	...
	xFrameOptions := rs.Header.Get("X-Frame-Options")
	if xFrameOptions != "deny" {
		t.Errorf("want: %q, got %q", "deny", xFrameOptions)
	}

	// Check that the middleware has correctly set the X-XSS-Protection header on the response.
	xXssProtection := rs.Header.Get("X-XSS-Protection")
	if xXssProtection != "1; mode=block" {
		t.Errorf("want %q, got %q", "1; mode=block", xXssProtection)
	}
